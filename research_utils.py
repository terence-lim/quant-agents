...
def standardize(x) -> pd.Series:
    ...
    Arguments:
        x: DataFrame with at least two columns, first column is the data to be standardized,
           last column is a boolean indicator for which rows to consider for standardizing
    Returns:
        pd.Series with the standardized values of the first column
    Usage:
        panel_frame.apply(standardize, indicator or True, fill_value=False)
    """
        ...
        if not month or cal.ismonth(next_date, month):
            ...
            for curr_date in cal.dates_range(prev_date, next_date):
                ...
                if curr_date in characteristics_dates:
                    ...
                    # If option is to ffill, or this month is the last hence must be picked up
                    if ffill or next_date == curr_date:
                        # stuff any observations into samples_df (later, will only keep last for each "next_date")
