...
from utils import as_nptype, LINKS, BENCHMARKS_RAG, CHARACTERISTICS_RAG, JKP_RAG_PATH, CRSP_RAG_PATH
...
# RAW DATA PATHS
DATA_LAKE = LINKS / "data"
BENCHMARKS_DIR = DATA_LAKE / "benchmarks"
CRSP_DATA = LINKS / "CRSP"
PSTAT_DATA = LINKS / "PSTAT"
...
def load_crsp(
    ...
    cap_panel = Panel(cap_df).save("TOTAL_CAP") ###
    
    count_df = df.groupby(level=0).apply(len).rename("TOTAL_COUNT")
    count_panel = Panel(count_df).save("TOTAL_COUNT") ###
...
if __name__ == "__main__":
    ...
    #
    ###########################
    def load_pstat_characteristics():
    ...
    #
    ###########################
    def load_jkp_benchmarks_and_characteristics():
