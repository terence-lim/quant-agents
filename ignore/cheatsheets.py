...
When I query the Performance Agent to write a research memo on a Panel containing stock characteristics data,
the Agent should respond by calling the Panel_factor_evaluate tool to retrieve summary evaluation statistics,
then append to and use the following instructions so it knows how to write the memo based on the summary statistics.
Please suggest the best way to modify the codebase.
Here’s the instructions, to which the summary statistics returned by the Panel_factor_evaluate tool
are appended, to provide as a context prompt so an AI agent knows how to write the research memo:
