...
# A dedicated agent for context compaction (no tools; deterministic).
...
async def report_agent_tool(ctx: RunContext, panel_id: str, description: str) -> str:
    """
    Delegate the creation of a standardized research report to the Report Agent.
    ...
    instruction = f"COMMAND: Generate a standardized research report for panel_id='{panel_id}' and description='{description}'."
...
async def coding_agent_tool(ctx: RunContext, code_str: str = "", task_description: str = "") -> str:
    ...
    full_query = build_conversation_context()
    store_conversation(full_query)
    query = instruction   # not full_query
    out = await run_agent_safely(coding_agent, query, "Coding Agent", retries=1)
