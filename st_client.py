...
and uses tools to perform quantitative analysis tasks on Panel data sets provided to you.
...
Use performance_agent_tool to create a research report on a Panel data set of stock characteristic values.
...
Use performance_agent_tool only to create a research report on a Panel data set of stock characteristic values.
""".strip(),
    model_settings={'temperature': 0.0, **model_parameters},
    toolsets=[common_server]
)
...
- Explain briefly how the cheat-sheet guided your solution.
- Write Python code that fulfills the request.
- Execute that code by calling the execute_python tool exactly once per attempt and report the result.
- Ensure the code returns either the name of the final `Panel` you constructed or a MEDIA file path for any plots.
""".strip(),
    model_settings={'temperature': 0.0, **model_parameters},
    toolsets=[coding_server, common_server]
)
...
async def run_agent_safely(agent, query: str, role_label: str, retries: int = 1) -> str:
    ...
    while True:
        try:
            ...
            attempt += 1
            logging.warning("Gemini returned empty content for %s (attempt %d): %s",
                            role_label, attempt, e)
            if attempt > retries:
